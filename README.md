...
に覚えている）。投入スクリプトは冪等で、**メモ（note）を消さない。** メモに触れるのは
出走しなかった候補を取り下げる（`withdrawn`）ときだけで、それも近況メモに移すだけ。

YAML は `pnpm run data:fetch` で netkeiba から書ける（候補・枠順・過去走・馬の基本情報・結果）。
Claude Code からは `/race-data` スキルで同じ手順を踏める。
