...
/** JST の今日を `YYYY-MM-DD` で返す。日付は JST 固定（design.md 第9章 #7）。 */
		...
		if (body) {
			statements.push(
				db
					...
					.onConflictDoUpdate({
						target: [note.authorId, note.raceEntryId],
			...
			statements.push(
				db.delete(note).where(and(eq(note.authorId, authorId), eq(note.raceEntryId, e.entryId)))
			);
			cleared++;
		}
	}
