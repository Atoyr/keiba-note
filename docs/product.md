...
- **D1 の制約** — **1リクエストあたりのクエリ数の上限は50**（Free plan の上限。Paid に上げた今も `wrangler.toml` の `[limits]` で50に置いている）。18頭分を扱うふりかえり画面で N+1 を書くと現実的に到達する。読みは JOIN、書きは `batch()` にまとめ、**1リクエスト10クエリ以内**を設計ルールとする（→ [architecture.md 第7章](./architecture.md#7-制約とスケール限界)）
