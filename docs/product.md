...
（打ち間違えると別のレースのオッズが付くため）。`field_size` も YAML（`data:fetch past` / `result`）からだけ入る。
...
│ │ 16頭 3枠5番 ルメール          1:58.4  5-5-4-2│ │
│ │ 26/06/28 G1 宝塚記念 芝2200m稍重 5着 4人気 上35.8│ │
│ │ 17頭 8枠16番 ルメール         2:12.1  8-8-7-6│ │
│ └──────────────────────────────────────────┘ │
...
**2行目に、左へ「どの枠から・誰が乗ったか」、右へタイムと通過順を出す。** どちらも補足の文字色で、
...
| --- | --- | --- |
| 日付 / レース名・格（条件戦はクラス）/ 馬場・距離・状態 / 着順 / 人気 / 上がり3F | 頭数 / 枠・馬番 / 騎手 / タイム / 通過順 | 斤量 / 馬体重 / 着差 / オッズ |
