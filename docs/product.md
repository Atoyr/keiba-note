...
- **独自ドメインと `*.workers.dev` の併存** — Cookie はホストごとなので、`uma-memo.com` と `k-note.<subdomain>.workers.dev` のセッションは別になる（両方でログインが要る）。移行はリダイレクト URI の追加だけで済んだ。workers.dev を止めるのは、独自ドメインで回ることを確かめてから
