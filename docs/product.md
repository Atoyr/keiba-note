...
- 更新日: 2026-09-25 — レース一覧（`/races`）を、何も指定せずに開いたら今年の重賞に絞るようにした（→ 第6章 `/races`）
- 更新日: 2026-09-26 — 予想画面の見立てに「展開の予想」（ペースと、スタート・4コーナー・ゴール前の隊列）を足した（→ 第5章 note / 第6章 `/races/[id]/preview`）
- ステータス: 確定（実装着手可）
...
通していては間に合わないので、ここだけは GitHub Actions が取得元から取って D1 に書く。

例外にしてよい理由と、そのための線引き:

- **マスタ（馬・レース・出走馬・結果）は今までどおり PR だけで入る。** Actions が書くのは表示専用の
...
- 重賞（G1〜G3）だけ。G1 は前々日の 18:30、G2・G3 は前日の 18:30 から、どちらも発走まで。30分おき（JST 7:00〜25:00。夜間販売があるので 25:00 まで取る）。L・OP・条件戦は取りに行かない。閲覧のたびに取得元へは行かない（画面は D1 の値を出す）
...
- 取得元は netkeiba（内部の JSON。公開の API ではない）。2026-09-24 に robots.txt を確かめた
  ...
  取得元を替えられるよう、取得元に固有の処理は `scripts/odds/netkeiba/` に閉じる。取りに行くのは GitHub Actions で、アプリ（Worker）は行かない
...
2026-09-26: 1メモずつの共有に加え、1レースの予想をまとめた共有を追加。
`/races/[id]/summary` で本人の保存済みの見立てと各馬のメモ・札・印を確認し、
明示的な共有操作でコピーを保存する。詳細は第6章「予想まとめ」。
...
| display_name | text NOT NULL | Google 由来の名前。本人のアカウント表示用で、共有には使わない |
| public_name | text NULL | 公開用の名前（30文字以内）。未設定は共有ページで「匿名」 |
...
| weather | text | |
| field_size | integer | 出走頭数（取消・除外を除く）。取得元の値。過去走は気にしている馬しか入らないので `race_entry` の行数では数えない |
| winner_name / runner_up_name | text | 勝ち馬・2着馬の馬名。頭数と同じ理由で `race_entry` からは引けないので名前で持つ |
...
（打ち間違えると別のレースのオッズが付くため）。`field_size`・`winner_name`・`runner_up_name` も YAML（`data:fetch past` / `result`）からだけ入る。
...
| last_3f | real | 上がり3F |
| time_diff | real | 勝ち馬とのタイム差（秒）。勝ち馬は2着との差を負で（`-0.2`）。`margin`（競馬の着差の表記）とは別 |
...
GitHub Actions が取得元から書く唯一のテーブル（→ 第1章「例外 — オッズ」）。1レース・1馬番につき1行で、履歴は持たない。
...
| flow | text NULL | 展開の予想（JSON）。`race_preview` にだけ付く（→ 下の「展開の予想（`flow`）」） |
...
見立て（`race_preview`）に、**ペース**（スロー・ミドル・ハイ）と、**スタート・4コーナー・ゴール前**の3局面の
隊列を付ける。隊列は盤面（前後10列 × 内外4段のマス目）のどこにどの馬がいるかで、局面ごとに一言メモも付く。
...
- **馬は馬番ではなく出走馬の id で持つ。** 予想は枠順の前に書くことが多く、そのときは馬番が無い。
  枠順が入れば置いた馬に馬番が付いて見える。取り下げで出走馬から消えた馬は、読むときに落ちる
- **本文が空でも展開だけで行を残す**（印・札と同じ扱い）。本文も展開も空なら行を消す
- **展開の欄が無い保存は、展開に触らない。** 展開の欄は出走馬がいるときしか出ないので、出走馬が0頭になった
  レースで見立ての本文だけ直しても、保存済みのペースとメモは残す（本文を空にしても、展開のある行は消さない）
- 形の正は `src/lib/schemas/race-flow.ts`。**「`race_preview` にだけ付く」は CHECK にしていない。**
  表の CHECK を足すには note 表を作り直すことになり、1列のために本番の中心の表を組み直すのは割に合わない。
  書くのはサービス層の見立ての経路（`raceNoteStatement`）だけ
- 文字の隊列記法（`⑤-③⑦-⑪`）で持たないのは、書いたあとに「どこに何を置いたか」を絵で見たいため。
  記法は盤面から組み立てて、畳んだ1行と読み上げに使う
...
/races                        レース一覧（日付降順。既定は今年の重賞 → 下の `/races`）
...
**レースが行で並ぶ一覧では、格の札（`GradeBadge`）をレース名の前に置く。**
名前だけではどの格の重賞か分からないので、名前を読む前に格が目に入るようにする。
ダッシュボードのレース欄・今週の重賞・レース一覧・馬柱のすべてでそろえ、**一覧ごとにずらさない**
（縦に並んだ行を目で追うとき、札の位置が一覧ごとに変わると目が迷う）。
条件戦のクラス（`1勝クラス` など）を札の代わりに出す一覧（レース一覧・馬柱）では、クラスも同じ位置に置く。
1レースを開いた画面の見出し（ふりかえり・予想の `RaceHeading`）と共有ページは並ぶ行ではないので、
この決まりの外に置く。見出しは札を2行目（条件の行）の先頭に（→ `RaceHeading` のコメント）、
共有ページはレースの行の末尾に出す。
...
│ 2026-09-26 中山11R G2 オールカマー   メモ 0  │
│ 2026-09-27 阪神11R G2 神戸新聞杯     メモ 3  │
├──────────────────────────────────────────────┤
│ ■ 過去のレース  直近3週                      │
│ 2026-09-20 中山11R G3 ○○S           メモ 2  │
...
- 3週より古いレースはここには出ない。探すのは `/races`。「すべてのレースを見る」は
  一覧の既定（今年の重賞）ではなく全件（`/races?year=`）へ向ける
- 格（G1〜G3・L・OP）のあるレースは、格の札（`GradeBadge`）を**レース名の前**に出す
  （一覧の決まり → この章の冒頭）。`grade` が NULL のレース（条件戦）には何も付けない

### `/races` — レース一覧

年度・ランク（G1〜OP の OR）・レース名（部分一致）で絞る。条件は GET のクエリで、
そのまま URL に残る（→ `$lib/utils/race-filter`）。

**何も指定せずに開くと、今年の重賞（G1〜G3）に絞る。** 一覧は日付降順なので、
絞らないと条件戦と、枠だけ先に登録した先の重賞が上を埋める。探しに来るのは
ほとんどが今年の重賞なので、そこから始める。L・OP は重賞ではないので既定には入れない。

- 既定になるのは、`year` `grade` `q` の**どれも URL に無いとき**だけ。フォームは年度を必ず送るので、
  「すべて」を選んでランクを外して送れば全件になる。どれか1つでも付いていれば、ほかを既定で補わない
- 「条件をクリア」は既定ではなく**全件**（`/races?year=`）へ。既定に戻すと、クリアしても絞られたままに見える
- 既定の状態もフォームにそのまま映す（年度に今年、G1〜G3 にチェック）。何で絞られているかは見れば分かる
- 今年のレースがまだ1つも無くても、年度の選択肢に今年を入れる（無いと「すべて」を選んだように見える）
- 既定のまま0件のときは「条件に合うレースがありません」と言わない。本人は条件を付けていないので、
  「今年の重賞はまだありません」と、全件へのリンクを出す（admin には登録の案内も）
...
一方、**一覧からのリンク先**は日付ではなく**結果が出ているか**で決める
（`isSettled`：開催済みで、着順の入った出走が1頭以上）。当日の朝に見立てを書こうと
開いたレースがふりかえりへ飛ぶと、書きたい画面に着かない。着順が入ったらふりかえりへ向ける。
今週の重賞・ダッシュボード（レースの行・注目馬・最近のメモ）・レース一覧・馬タイムラインの
すべてでこの線引きにそろえ、**画面ごとにずらさない**。ダッシュボードの進み具合の札と
「ふりかえり待ち」も同じ線引きで、「ふりかえり待ち」と出ているのに押すと予想画面へ行く、
という食い違いを作らない。

ただし**自分がふりかえりを書いてあるレース**（「ふりかえり済」の行・ふりかえりのメモ）は、
結果の投入前でもふりかえりへ向ける（`opensReview`）。予想画面にふりかえりは出ないので、
押して書いたものが無い画面に着くことになる。

この画面が開ける・開けないの線引きは日付（上の `isUpcoming`）のまま変えない。
結果の投入が遅れた日でも、予想画面の「ふりかえりを書く」から入って書けるようにするため。

```
┌──────────────────────────────────────────────┐
│ 中山11R  オールカマー                        │
│ G2 2026-09-20 · 芝2200m / 右 / 良 / 晴       │
├──────────────────────────────────────────────┤
│ ■ レースのメモ                               │
│ ┌──────────────────────────────────────────┐ │
│ │ 前半緩くて上がり勝負。内有利。           │ │
...
| 問題 | 原因 | 対策 |
| --- | --- | --- |
| 書き忘れ | 保存していないことに気づけない | 未保存のときだけ出る保存ボタン（件数つき） ＋ 離脱時の確認 |
...
**保存ボタンは未保存の変更があるときだけ出す**（`SaveBar.svelte`）。いつも出ていると、
書いたものが保存済みかを画面から読めない。ボタンが出ていること自体を「保存していないものがある」の合図にする。
保存が通ると「保存しました」をトーストで出し、ボタンは消える。JS が無いときは件数を数えられないので、ボタンは常に出す。

**件数の1件はメモ1つ**（レースのメモ、または1頭ぶん）。本文・札・印と欄が分かれていても、1頭なら1件。
保存の知らせの「（N 件）」も同じ数え方で、**その保存で変えたメモの数**を出す。押す前の
「未保存の変更が N 件」と数が変わると、何か操作を間違えたのかと読ませてしまう。

**離脱時の確認は、アプリ内のリンクでも出す。** SvelteKit のリンクは読み込み直さないので
`beforeunload` が起きない。`beforeNavigate` でも止める。同じ画面の中のアンカー（付けた印から馬の行へ）は止めない。
...
見出しはふりかえりと同じ形にする（1行目に場・R とレース名、2行目に格の札・日付・条件。
組み立ては `src/lib/utils/race-heading.ts` を共有する）。予想だけの違いは、条件の末尾に頭数を足すこと。
戻り先の「今週の重賞」へのリンクは置かない（上のナビにある）。

見出しのすぐ下に**付けた印**と**コース図**（`CourseMap`）を並べる。広い画面では左に印・右にコース、
スマホでは縦に積み、コースは畳んでおく（見出しの行に回り・直線・高低差だけを出し、開くと図が出る）。
片方しか無いときは全幅にする。
付けた印は印の順（◎ → ×）に並べ、広い画面では1頭1行にする。スマホでは折り返して詰める（縦に長くしない）。

```
┌────────────────────────────────┐
│ 京都11R ○○賞                    │
│ G2 2026-10-04 · 芝2200m / 16頭   │
├───────────────┬────────────────┤
│ 付けた印       │ コース（図）    │
├───────────────┴────────────────┤
│ レースの見立て                  │
...
コース図は、そのレースの馬場（芝・ダート・新潟の直線）を色で、ほかのコースを薄い灰色で描き、
回り・直線の長さ・高低差を文字で添える（色だけに頼らない）。
**内回り・外回りのある場（新潟・中山・京都・阪神）は距離で出し分ける。** 表は JRA の「コース紹介」の
発走距離のまま持ち、どちらか一方にだけ載っている距離はそのコースだけを強く描いて寸法もそのコースのものにする。
両方に載っている距離のうち、実際にはほぼ一方で行う京都2000m・阪神1400m は内回りに寄せる。
残りの両方にある距離（京都1400・1600m、新潟1400・2000m）と、中山・阪神の3200m（外回りから内回りへ回る）は決められないので、
両方を描いて寸法も両方出す。
寸法は JRA の「コース紹介」の A コースの値。図は形と大きさの見当をつけるための模式図で、
**JRA のコース図は写さない**（著作物）。全場で同じ縮尺にしてあり、東京と札幌の大きさの違いが図でも分かる。
スタート地点と坂の位置は描かない。
ふりかえり画面にも同じ図を置く（答え合わせの下、書く欄の直前。スマホでは同じく畳む）。
JRA の10場以外と、馬場が決まっていないレースには出さない。
...
**見立ての下に「展開の予想」を畳んで置く**（出走馬がいるときだけ。盤面に置く馬がいないため）。
見続けるものではないので既定は閉じ、閉じた行にペースと隊列の1行（`スタート ⑤-③⑦ → 4角 … → ゴール前 …`）だけを出す。
開くとペース・局面のタブ（スタート／4コーナー／ゴール前）・盤面・まだ置いていない馬・一言メモが出る。

- **置き方はタップだけ。** 馬を押して選び、置くマスを押す。置いたコマを選んで別のマスを押すと動き、
  別の馬のマスなら入れ替わる。スマホでドラッグに頼らない
- 盤面は上が内ラチで、**先頭はスタンドから見た向き**（右回りは左、左回りは右）。中継の画面と同じ向きにして、
  頭の中の絵と取り違えないようにする。回りが入っていないレースは場の回り（コース図と同じ出どころ）で決め、直線コースと場も分からないレースは先頭を左にする
- 4角・ゴール前は「前の局面の並びを写す」から始められる（スタートの並びを少し動かすことが多い）
- コマは枠の色（`BracketBadge` と同じ）に馬番。馬番が無いうちは馬名の頭2文字（まだ置いていない馬の一覧と同じ字）
- 保存は「まとめて保存」に乗り、下書き（`DraftKeeper`）にも入る
- キーボードでは盤面が1つの止まり場所で、矢印キーでマスを移る（40マスを Tab で辿らせない）
- 予想まとめ・共有ページ・ふりかえりの「開催前の見立て」でも**畳んで出す**。閉じた行はペースと隊列の1行で、
  開くと3局面の盤面が並ぶ。開いたまま置くと、スマホでは盤面が 700px を超えて各馬の印を1画面目から押し出す
  （答え合わせのため。ここからは直せない）
...
│ 3  ⑤ ホースA  ルメール                  ◎   │
│ ┌──────────────────────────────────────────┐ │← 馬柱（薄い面に載せる）
│ │ 26/08/23 G2 札幌記念 芝2000m良 1着 2人気 上34.2│ │
│ │ 16頭 3枠5番 ルメール ホースB（-0.2）1:58.4 5-5-4-2│ │
│ │ 26/06/28 G1 宝塚記念 芝2200m稍重 5着 4人気 上35.8│ │
│ │ 17頭 8枠16番 ルメール ホースC（0.4）2:12.1 8-8-7-6│ │
│ └──────────────────────────────────────────┘ │
...
その場で使えなくなる。1行目に出すのは「いつ・何で・どう走ったか」に絞る。

**日付は `26/08/23`（YY/MM/DD）。** 休み明けで5走が年をまたぐと、月日だけでは前走からの間隔も、
何年前の走りかも読めない。

**2行目に、左へ「どの枠から・誰が乗ったか・誰にどれだけ負けたか」、右へタイムと通過順を出す。** どちらも補足の文字色で、
右の組は着順のまとまりの下に揃える。1行目に足すと狭い画面で折り返しが増え、着順の位置が行ごとにずれて
縦に拾い読みできなくなる。どれも入っていない走は2行目ごと出さない。

頭数は `race.field_size`（取得元の出走頭数）で、**`race_entry` の行数では数えない。** 過去走は
気にしている馬しか入っていないので、数えると「2頭」のような値になる。頭数が入っていない走は枠・馬番だけを出す。

**負けた走は勝ち馬、勝った走は2着馬を、タイム差と一緒に出す**（`ホースC（0.4）` / `ホースB（-0.2）`。新聞の馬柱と同じ）。
着順の無い走（取消・中止・結果が未入力）には出さない（勝ち馬を出すと負けたように読める）。
タイム差は取得元（netkeiba の戦績表）の表記のまま秒で持ち、勝った走だけが負になる。競馬の着差の表記（`クビ`）は出さない。
勝ち馬と2着馬も頭数と同じ理由で `race` に名前で持つ。

| 1行目 | 2行目 | 出さない（DB には入れる） |
| --- | --- | --- |
| 日付 / レース名・格（条件戦はクラス）/ 馬場・距離・状態 / 着順 / 人気 / 上がり3F | 頭数 / 枠・馬番 / 騎手 / 勝ち馬（勝った走は2着馬）とタイム差 / タイム / 通過順 | 斤量 / 馬体重 / 着差（競馬の表記）/ オッズ |

**重賞に限らない。** 格が無いレースは `class_name`（`1勝クラス` 等）を代わりに出す。
重賞は格の札で足りるが、**条件戦は条件そのものがレースの識別子**になる。
重賞で絞っているのは `/this-week` と、レース一覧の既定（外せる → `/races`）だけで、馬柱にその絞りは無い
...
- 出すのは GitHub Actions が D1 に置いた値だけ。この画面から取得元へは行かない
...
出すもの: メモ本文、札／印、日付、文脈（レース名・日付・馬名・着順）、著者が設定した公開用の名前（未設定は匿名）。
...
- `/races/[id]/summary` は本人だけの閲覧画面。予想画面とふりかえり画面から開ける。
  レース全体の見立て（展開の予想を含む）と、予想を保存した馬の本文・札・印を、印順（◎ → ○ → ▲ → △ → ☆ → × → 印なし）、同じ印は馬番の昇順にまとめる。
  共有済みのページも表示時に同じ順序へ揃える。馬番未確定は同じ印の最後に置く。
  ふりかえりや過去メモ、他人の予想は含めない。印なしのメモも表示し、印だけ・見立てだけでも使える。
  「予想まとめ」と同じ行の右端に「予想」とシェアアイコンを置く。「予想」で入力画面へ戻る。
  シェアアイコンは共有リンクを作成・更新して端末の投稿先選択（Web Share API）を開く。
  非対応端末ではリンクをコピーし、コピー不可ならリンク欄を案内する。共有画面を閉じてもリンクは有効。
  通信待ちで投稿先選択を開けなかった場合は、発行済みリンクで「投稿先を選ぶ」を再試行できる。
- 「共有リンクを作る」で `race_share` に共有用コピーを保存し、`/shared/races/[id]` を発行する。
  元のメモの `visibility` は変えない。あとからの編集・削除を自動で公開せず、
  シェアアイコンか「共有内容を更新」で同じURLに反映する。現在の予想とコピーが違う場合は更新の案内を出す。
- 共有解除後は404。再共有は新しいURLになり、取り消したURLは復活しない。
  `/settings/shares` に単体メモと予想まとめの両方を載せ、取り消せる。
- `/settings/profile` で公開用の名前を設定する。前後の空白を除去し30文字以内、空欄なら匿名。
  Google の名前・メール・画像を共有ページの表示にもデータにも含めない。
  公開名の変更は既存の単体メモ・予想まとめにも反映する。
- 共有ページはログイン不要で、本人が開いても同じ表示。検索抑止・キャッシュ抑止は単体メモ共有と同じ。
...
| 12 | 共有ページに著者名を出すか | **公開用の名前だけ出す。** `public_name` 未設定は「匿名」。Google名・email・アバターはデータにも出さない |
...
- **D1 の制約** — **1リクエストあたりのクエリ数は Free plan で50が上限だった**（Paid に上げた今は遠いが、約束は変えない）。18頭分を扱うふりかえり画面で N+1 を書くと現実的に到達する。読みは JOIN、書きは `batch()` にまとめ、**1リクエスト10クエリ以内**を設計ルールとする（→ [architecture.md 第7章](./architecture.md#7-制約とスケール限界)）
