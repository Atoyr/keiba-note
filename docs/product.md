...
│ 3  ⑤ ホースA  ルメール                  ◎   │
│ ┌──────────────────────────────────────────┐ │← 馬柱（薄い面に載せる）
│ │ 08/23 G2 札幌記念 芝2000m良  1着 2人気 上34.2│ │
│ │                               1:58.4  5-5-4-2│ │
│ │ 06/28 G1 宝塚記念 芝2200m稍重 5着 4人気 上35.8│ │
│ │                                2:12.1  8-8-7-6│ │
│ └──────────────────────────────────────────┘ │
...
**タイムと通過順は2行目に出す。** 着順のまとまりの下に右寄せで、補足の文字色で置く。
1行目に足すと狭い画面で折り返しが増え、着順の位置が行ごとにずれて縦に拾い読みできなくなる。
どちらも入っていない走（結果が未入力）は2行目ごと出さない。

| 1行目 | 2行目 | 出さない（DB には入れる） |
| --- | --- | --- |
| 日付 / レース名・格（条件戦はクラス）/ 馬場・距離・状態 / 着順 / 人気 / 上がり3F | タイム / 通過順 | 斤量 / 馬体重 / 着差 / オッズ |
