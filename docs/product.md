...
ただし `/this-week` の**リンク先**だけは、日付ではなく**結果が出ているか**で決める
（`isSettled`：開催済みで、着順の入った出走が1頭以上）。今週の重賞は予想の入口で、
当日の朝に見立てを書こうと開いたレースがふりかえりへ飛ぶと書きたい画面に着かない。
着順が入ったらふりかえりへ向ける。この画面が開ける・開けないの線引きは上のまま変えない。
