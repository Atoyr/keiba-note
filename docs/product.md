...
**格の札（`GradeBadge`）は、レース名を出すどの画面でもレース名の前に置く。**
名前だけではどの格の重賞か分からないので、名前を読む前に格が目に入るようにする。
ダッシュボード・今週の重賞・レース一覧・レースの見出し（ふりかえり・予想）・馬柱・共有ページの
すべてでそろえ、**画面ごとにずらさない**（札を探す位置が画面ごとに変わると、目が迷う）。
条件戦のクラス（`1勝クラス` など）を札の代わりに出す一覧（レース一覧・馬柱）では、クラスも同じ位置に置く。
レースの見出しだけは例外で、クラスは2行目の条件の先頭に出す。見出しの1行目は折り返さず
名前を … で詰めるので、札より長いクラスを前に置くと、スマホ幅で名前がほとんど読めなくなる。
...
- 格（G1〜G3・L・OP）のあるレースは、格の札（`GradeBadge`）を**レース名の前**に出す
  （全画面の決まり → この章の冒頭）。`grade` が NULL のレース（条件戦）には何も付けない
