...
| 起動 | 入口 | すること |
| --- | --- | --- |
| `*/30 0-8 * * *`（UTC。JST 9:00〜17:30 の30分おき） | `src/worker.js` の `scheduled` → `lib/server/odds/scheduled.ts` | 当日・発走3時間前〜発走のレースのオッズを取得元から取り、`race_odds` に書く（→ [architecture.md 3-8](./architecture.md)） |
