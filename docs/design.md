...
── 要ログイン ────────────────────────────────────────────
/                             ダッシュボード（**自分の**最近のメモ / 直近のレース＝次のレースから5件）
/auth/logout                  ログアウト（POST のみ）
/this-week                    今週の重賞（連休は火曜まで含む → 第9章 #15）
/races                        レース一覧（次のレースから → 第6章）
...
│ ■ タイムライン          [+ 近況メモを追加]   │
│                                              │
│ 2026-10-25  [出走予定] 東京11R 天皇賞(秋)    │← 次走。メモはまだ無い
│ 2026-12-27  [出走予定] 中山11R 有馬記念      │← その先の予定は下に続く
│                                              │
...
**並びは今日を起点に近い順。** 未来が上・過去が下で、**どちらの側も今日に近いほど上**に出る
（未来は昇順、過去は降順）。馬を追う理由は「次どこに出るか」なので、先頭は常に次走にする。
単純な降順にすると、いちばん先の予定が先頭に立ち、肝心の次走が未来の末尾＝過去との境目に埋もれる。
...
### レース一覧の並び（`/` の「直近のレース」と `/races`）

**今日を起点に、次のレースから並べる。** 未来が先で近い順、そのあと過去が新しい順。
同じ日なら未来は R の若い順（次に始まる1本）、過去は R の遅い順（最後の1本）。
馬タイムラインと同じ規則で、比較子は `compareNextFirst`（`src/lib/utils/date.ts`）に1つだけ置く。

日付の降順にすると、**開催のずっと前に枠だけ登録した重賞が上を埋める。**
ダッシュボードは先頭5件しか出さないので、次のレースが一覧から溢れることさえあった。

並べ替えは JS 側でやる。SQL の ORDER BY に書くと未来・過去で向きが変わる CASE が並んで
読めるものにならない。**取ってくる窓は日付の降順のまま**（新しい側から100件）で、
ここを崩すと過去が先に切れる。

`/races` の絞り込み（年度・格付け・レース名）をかけても並べ方は変わらない。
絞り込みは WHERE で件数を減らすだけで、残った中で「次のレース」が先頭に来る。
