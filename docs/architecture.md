...
sequenceDiagram
    autonumber
    participant C as Cron（*/30 0-13 * * * UTC）
...
- 対象は YAML に `ref` と `startTime` を書いたレースだけ。取りに行く時間帯は格で決まる（`src/lib/utils/odds.ts`）

  | 格 | 取りに行き始める | 1レースの回数（日曜 15:40 発走の例） |
  | --- | --- | --- |
  | G1 | 前々日の 18:30（金曜から売る G1 がある） | 金 9回 + 土 28回 + 日 14回 = 51回 |
  | G2・G3 | 前日の 18:30（前日発売のオッズが出始める頃） | 土 9回 + 日 14回 = 23回 |
  | それ以外 | 当日の発走3時間前 | 6〜7回 |

  どれも発走まで。Cron は JST 9:00〜22:30 だけ回し、夜中は取りに行かない。発売前は取得元が予想オッズしか返さず、
...
- オッズ（3-8）: G1 1つ・G2/G3 2つの週末として、(51 + 23 × 2) 回 × 18頭 ≒ 1,800行の書き込み（`race_odds` は
