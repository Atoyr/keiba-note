...
- 1リクエストの D1 クエリは10以内（`wrangler.toml` の `[limits] subrequests` で50を上限に置いている）。超えそうなら JOIN か `batch()` にまとめる（→ 7-1）
...
### 7-1. 1リクエストあたりの D1 クエリ数 — 上限 50（`[limits] subrequests`）
...
超えそうなら JOIN か `batch()` にまとめる。これを守っていれば上限の50は遠い。
Paid の既定（10,000）まで上げずに50に置いているのは、N+1 の暴走を課金ではなくエラーとして表に出すため。

### 7-2. CPU 時間 — 上限 100ms / 呼び出し（`[limits] cpu_ms`）
...
ただし Worker ごと止められるので Discord には届かない。Workers Logs で見る（→ [monitoring.md 9-1](./monitoring.md)）。
